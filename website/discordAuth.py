...
class DiscordAuth(object):
    ...
    discord_webhook_url = DISCORD_API + \
        "webhooks/{}/{}".format(webhook_id, webhook_secret)
    ...
    def get_access_token(code, redirect):
        ...
        access_token = post(url=DiscordAuth.discord_token_url,
    ...
    async def sendRequest(user, data):
        avatar_url = DiscordAuth.getProfileImage(
            user.get("id"), user.get("avatar"))
        ...
        async with aiohttp.ClientSession() as session:
            webhook = Webhook.from_url(
                DiscordAuth.discord_webhook_url, adapter=AsyncWebhookAdapter(session))
