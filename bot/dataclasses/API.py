...
class Api(object):
    ...
    def postVote(positive, negative):
        builder = f"{Api.URL}sendvote?{secret}&positive={positive}&negative={negative}"
        return requests.post(builder)

    @staticmethod
    def startVote( id, avatar, name):
        builder = f"{Api.URL}startvote?{secret}&id={id}&avatar={avatar}&name={name}"
        return requests.post(builder)
    
    @staticmethod()
    def sendInvite(id, inviteLink):
        builder = f"{Api.URL}{secret}&id={id}&invite={inviteLink}"
